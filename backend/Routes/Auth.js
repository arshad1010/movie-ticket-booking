...
router.post('/login', async (req, res, next) => {
    ...
    const authToken = jwt.sign({ userId: user._id }, '1234', { expiresIn: '10m' });
    const refreshToken = jwt.sign({ userId: user._id }, '1234', { expiresIn: '30m' });
...
router.get('/checklogin', authTokenHandler, async (req, res) => {
    console.log("check login backend")
    console.log(req.body)
    res.json({
        userId: req.userId,
        ok: true,
